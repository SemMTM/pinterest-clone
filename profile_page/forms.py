...
class ProfileForm(forms.ModelForm):
    class Meta:
        ...
        def clean_profile_image(self):
            ...
            if not image or isinstance(image, str) or hasattr(image, "public_id"):
                return image
            ...
            if extension not in allowed_extensions:
                raise forms.ValidationError(f"Invalid file type: {extension}. Allowed: {', '.join(allowed_extensions)}")

            # Validate file size
            max_file_size_mb = 2  # Set max file size
            file_size_mb = image.size / (1024 * 1024)  # Convert bytes to MB
            if file_size_mb > max_file_size_mb:
                raise forms.ValidationError(f"File size exceeds {max_file_size_mb}MB limit.")
